  ...
  return through2.obj(function (file, enc, done) {
      ...
      async.each(matched, function(conf, cb) {
        ...
        sharpVinyl(file, conf, options, function(err, newFile) {
          ...
          if (newFile) {
            // these stats aren't actually perfectly accurate and won't
            // be used by gulp.dest, but at least Stats will be there if other gulp functions
            // need it.
            newFile.stat = file.stat;
            that.push(newFile);
            statistics.created++;
          }
          cb();
